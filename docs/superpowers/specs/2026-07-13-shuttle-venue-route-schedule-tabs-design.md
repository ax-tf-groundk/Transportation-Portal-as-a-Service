...
### 4.1 노선도 — 기존 `.route-map` 컴포넌트 재사용, 방향별 2개(가는 편/오는 편), 노선당 1쌍(날짜 무관)

`aphrs-2026-transport/airport-transfer.html`이 이미 쓰고 있는 `.route-map`/`.rm-track`/`.rm-line`(`grid-template-columns`로 정류장 수만큼 칸 분할)/`.rm-stop`/`.rm-dot`/`.rm-label`(-58deg 회전 라벨)을 **그대로** 재사용한다. 정류장 3개는 이미 검증된 범위(기존 사용처는 13~15개) 안이라 그대로 3칸 그리드로 렌더된다 — U자형 곡선(1차 개정 폐기)은 여전히 불필요. BEXCO 표시도 `airport-transfer.html`의 기존 선례(`<span class="rm-label" style="color:var(--red)">BEXCO</span>`, 별도 확대 점 없이 빨간 라벨만)를 그대로 따른다 — `.rm-airport`(공항/역 전용 확대 점 + 점선 연결) 클래스는 재사용하지 않는다(연결선이 점선으로 바뀌는 부수효과가 이 노선엔 안 맞음).

노선명 배지 + "실시간 위치 확인" 버튼을 위해 `.rm-head`(신규, 최소 CSS 1줄)만 추가하고, 배지는 기존 `.rt-tag`(다른 곳에서 이미 쓰이는 노선 배지 pill), 버튼은 기존 `.rm-live`(`transport.css`에 정의만 돼 있고 실사용은 없던 펄스 애니메이션 배지)를 그대로 사용한다. 가는 편·오는 편은 **완전히 독립된 두 블록**으로 그린다(하나로 합치거나 "왕복" 캡션으로 대체하지 않음 — 방향별 표현은 반드시 분리 유지).
...
<!-- vr-b는 웨스틴조선/파크하얏트/BEXCO, rt-tag 텍스트만 "B"로 동일 구조 반복 -->
```

- `vd-*` 클래스 없음 — 노선당(A/B) 딱 1쌍(가는 편+오는 편)씩, 총 4개 노선도만 DOM에 존재. 날짜 탭 클릭은 이 블록에 영향 없음(1차 개정과 동일).
- **신규 JS 없음** — `.route-map`은 순수 CSS Grid라 렌더 로직이 필요 없다. 탭 전환으로 숨겨졌던 블록이 나중에 보일 때 폭이 잘못 계산되는 문제 자체가 발생하지 않는다(SVG `clientWidth` 이슈 없음).
- **신규 CSS**: `.rm-head{display:flex;align-items:center;gap:10px;flex-wrap:wrap;padding:0 4px 10px}` 한 줄만 추가. `.uroute`는 기존 카드 스타일(배경·보더·그림자)을 버리고 `display:flex;flex-direction:column;gap:18px`로 단순화(각 방향이 `.route-map` 자체 카드 스타일을 이미 갖고 있어 이중 카드 방지 — `.route-map`은 이 프로젝트에서 항상 단독 카드로만 쓰여온 관례라 이쪽에 맞춤). `.uroute-arm`은 `display:flex;flex-direction:column;gap:8px`.
- **배지 색 구분**: 가는 편 `.rt-tag`는 기본값(`--navy`), 오는 편은 인라인 `style="background:var(--red)"` — `.sched-dir-h`가 이미 같은 파일에서 쓰는 "인라인 style로 방향 색 구분" 관례를 그대로 따름(신규 CSS 클래스 불필요).
...
- 위(가는 편)·아래(오는 편) 블록 사이 간격은 `.uroute{gap:18px}`로 처리(구분선 없이 카드 간 여백만).
- **시각 검증 포인트**(구현 후 스크린샷으로 확인): 정류장 3개일 때 라벨 회전(-58deg)이 겹치거나 잘리지 않는지, `.rm-head`의 `.rt-tag`+텍스트+`.rm-live` 한 줄 배치가 좁은 화면(390px)에서 줄바꿈되는지, `.uroute` 단순화 후 두 방향 카드 사이 여백이 어색하지 않은지.
