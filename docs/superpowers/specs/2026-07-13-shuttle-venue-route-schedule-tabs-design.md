...
>
> **2026-07-14 5차 개정 배경**: Task 4~5(시간표 통합) 완료 후 사용자가 다른 프로젝트의 화면 레이아웃(실제 지명·문구는 무관, 구조만 참고)을 보여주며 정보구조 변경을 요청 — 지금은 노선당 날짜 탭이 1개뿐이라 "행사장행"/"호텔행" 두 표가 날짜마다 번갈아 나열되는데, 이걸 방향별로 완전히 분리된 섹션(각자 자기 노선도 + 자기 날짜 탭 + 자기 4일치 표)으로 바꿔달라는 것. **날짜 상태(`data-vd`) 자체는 여전히 전역 하나** — 방향마다 독립된 날짜를 고르는 게 아니라 날짜 탭 버튼의 물리적 위치만 방향 섹션마다 복제해서 배치(기존 `applyGroup` JS가 버튼 개수와 무관하게 이미 전부 동기화하므로 신규 로직 불필요). 시간표 데이터 자체(Task 4·5의 결과물)는 전혀 바뀌지 않고, 16개 `.sched-dir` 블록의 물리적 순서(날짜별 인터리브 → 방향별 그룹)와 날짜 탭 바의 개수(1개 → 4개, 노선×방향별)만 바뀐다. 아래 §3.2·§4.2는 이 5차 개정을 반영한 최종본이다(§4.1·§6·§7·§8은 4차 개정본 그대로 유효).
>
...
### 3.2 탭 UI 레이아웃 (2026-07-14 5차 개정 — 날짜 탭을 방향별 섹션 안으로 분리)

```
[ A 노선 ]  [ B 노선 ]          ← 노선 탭 (.vtabs)

┌─ 행사장행 노선도(route-map, 날짜 무관 고정) ─────────────┐
│  행사장행   실시간 위치 확인(준비중)                      │
│  ●───────●───────◉   (route-map: 점+선+대각선 라벨)      │
└────────────────────────────────────────────────────────────┘
[10/21] [10/22] [10/23] [10/24] ← 행사장행 전용 날짜 탭(vr-a 사본)
운영시간 안내 문구 (날짜별) + 행사장행 시간표 ×4일치(집중/일반/리셉션 통합 1개 표)

┌─ 호텔행 노선도(route-map, 날짜 무관 고정) ───────────────┐
│  호텔행     실시간 위치 확인(준비중)                      │
│  ◉───────●───────●   (역방향 — 별도 route-map 카드)      │
└────────────────────────────────────────────────────────────┘
[10/21] [10/22] [10/23] [10/24] ← 호텔행 전용 날짜 탭(vr-a 사본, 위와 동일 전역 data-vd)
호텔행 시간표 ×4일치(안내 문구는 행사장행 쪽에만, 중복 노출 안 함)

(10/23 한정, 노선 무관 공통 노출 — 호텔행 그룹 다음, 위치 불변)
┌─ Faculty Dinner 셔틀 안내 + 시간표 ──────┐
└──────────────────────────────────────────┘
```

- **날짜 탭이 노선×방향마다 1개씩, 총 4개** 존재(행사장행 vr-a / 호텔행 vr-a / 행사장행 vr-b / 호텔행 vr-b) — 전부 같은 `data-vd-btn` 마크업의 복제본. 상태는 여전히 `<html data-vd="...">` 전역 값 하나뿐이라, 어느 탭바를 클릭해도 두 탭바(같은 노선 안 행사장행·호텔행)가 동시에 갱신된다(기존 `applyGroup` JS가 `document.querySelectorAll('[data-vd-btn]')`로 이미 전부 동기화).
- 노선 탭 전환(vr-a↔vr-b)은 각 노선의 날짜탭 2개+시간표 8개(4행사장행+4호텔행)를 통째로 `display:none` 토글 — 노선도 카드와 동일한 `vr-a`/`vr-b` 게이팅 재사용, 신규 CSS 없음.
- Faculty Dinner 블록은 `vd-1023` 클래스만 가지고 `vr-a`/`vr-b` 어느 쪽에도 속하지 않음 → 날짜만 10/23이면 노선 선택과 무관하게 노출, 위치는 호텔행 그룹(노선 B) 바로 다음으로 변경 없음.
...
- **(5차 개정)** 물리적 DOM 순서가 "날짜별로 가는편·오는편 인터리브"에서 "방향별로 묶고 그 안에서 날짜순"으로 바뀐다(§3.2 참조) — 노선당 8개 `.sched-dir`가 이제 [가는편 4개(10/21~24) → 오는편 4개(10/21~24)] 순서로 재배열된다. `sched-note`(운영시간 안내)는 가는편 그룹에만 유지하고 오는편 그룹에는 반복하지 않는다(중복 안내 문구 페이지당 1회 원칙). 데이터·회차·`vr`/`vd` 게이팅 클래스 자체는 전혀 바뀌지 않는다 — 순서만 바뀐다.
